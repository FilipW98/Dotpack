...
//   if ($ext === 'svg') {
//       // Zamiana protokołu na HTTP, jeśli URL zaczyna się od HTTPS
//       if (strpos($url, 'https://') === 0) {
//           $url = str_replace('https://', 'http://', $url);
//       }

//       // Pobranie zawartości pliku SVG
//       $svg_content = file_get_contents($url);
//       if ($svg_content !== false) {
//           echo $svg_content;
//       } else { 
//           echo '<!-- SVG nie został pobrany -->';
//       }
//   } else {
...
function print_svg($image_id){
  ...
  if ($ext === 'svg') {
      ...
      // Debugowanie: wyświetlenie ścieżek
      // echo '<!-- URL: ' . esc_url($url) . ' -->';
      // echo '<!-- Lokalna ścieżka: ' . esc_html($local_path) . ' -->';

      if (file_exists($local_path)) {
          echo file_get_contents($local_path);
      } else {
          echo '<!-- Plik SVG nie istnieje w lokalnej ścieżce: ' . esc_html($local_path) . ' -->';
      }
  } else {
      echo '<img src="' . esc_url($url) . '" alt="">';
  }
